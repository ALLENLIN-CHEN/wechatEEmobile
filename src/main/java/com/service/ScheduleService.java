...
/*    public Pager findConferenceStatisticsForSubproject(){
    ...
    public ArrayList findStatisticsForSubproject(Pager pager,Integer teamId, String scheduleType,String searchingForSheduleName, String startTime, String endTime){
        ArrayList <Map> arrayList=new ArrayList<Map>();
        Pager scheduleList=scheduleDao.findStatisticsForSubproject(pager,teamId,scheduleType,searchingForSheduleName,startTime,endTime);
        for(int i=0;i<scheduleList.getDataList().size();i++){
            Object []row=(Object [])scheduleList.getDataList().get(i);
            Map<String,Object> scheduleMap=new HashMap<String, Object>();
            scheduleMap.put("scheduleId",row[0]);
